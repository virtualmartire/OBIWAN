...
class WeightedRadialAEVComputer(tf.keras.layers.Layer):
   ...
   def __init__(self,
      ...
      self.MuR = tf.constant(radial_AEV_params["MuR"], dtype=self.dtype)

      self.pairs_params_r = self.doubleCartProd(self.EtaR, self.MuR)
   ...
   def GRFormula(self, interesting_distances, interesting_z_j):
      """The ANAKIN AEV radial term (equal to the original Gastegger one)."""
      ...
      EtaR, MuR = tf.unstack(pairs_params_r, axis=-1)

      return z_j * tf.math.exp(-EtaR * (R_ij - MuR)**2) * self.f_C(R_ij, self.Rcr)
...
class WeightedAngularAEVComputer(tf.keras.layers.Layer):
   ...
   def __init__(self,
      ...
      self.Lambda = tf.constant(angular_AEV_params["Lambda"], dtype=self.dtype)
      self.EtaA = tf.constant(angular_AEV_params["EtaA"], dtype=self.dtype)
      self.MuA = tf.constant(angular_AEV_params["MuA"], dtype=self.dtype)

      self.quadruplets_params_a = self.quadrupleCartProd(self.Zeta, self.Lambda, self.EtaA, self.MuA)
   ...
   def carnotTheorem(distances):
      ...
      return tf.stack([alpha, R_ij, R_ik, R_jk], axis=1)    # returns also the interesting distances
   ...
   def GAFormula(self, interesting_geometries, interesting_species_jk):
         ...
         angle, R_ij, R_ik, R_jk = tf.unstack(interesting_geometries, axis=-1)
         z_j, z_k = tf.unstack(interesting_species_jk, axis=-1)
         Zeta, Lambda, EtaA, MuA = tf.unstack(quadruplets_params_a, axis=-1)

         factor_1 = ( (1 + Lambda*tf.math.cos(angle)) / 2 ) ** Zeta
         factor_2 = tf.math.exp(-EtaA * (R_ij - MuA)**2)
         factor_3 = tf.math.exp(-EtaA * (R_ik - MuA)**2)
         factor_4 = tf.math.exp(-EtaA * (R_jk - MuA)**2)
         GA_addends = z_j*z_k * 2 * factor_1 * factor_2 * factor_3 * factor_4 * self.f_C(R_ij, self.Rca) * self.f_C(R_ik, self.Rca) * self.f_C(R_jk, self.Rca)
   ...
   def call(self, distance_matrices, atomic_numbers_batch):
      ...
      cutoff_mask = tf.reduce_all(triplets_distances < [self.Rca, self.Rca, self.Rca], axis=-1) & tf.reduce_all(triplets_distances != [0., 0., 0.], axis=-1)
...
class WAnakin(tf.keras.Model):
   """Reproducing the Gastegger paper."""

   def __init__(self, max_molecule_size):
      super(WAnakin, self).__init__(name="w_anakin")

      radial_AEV_params = {
                           "Rcr": 5.1000e+00,         # we used the anakin cutoff (here and below)
                           "EtaR": [6.3775510],
                           "MuR": [0.5, 0.78, 1.06, 1.34, 1.62, 1.9, 2.18, 2.46, 2.74, 3.02, 3.3, 3.58, 3.86, 4.14, 4.42 , 4.7, 4.98, 5.26, 5.54, 5.82, 6.1, 6.38, 6.66, 6.94, 7.22, 7.5]
                        }
      angular_AEV_params = {
                              "Rca": 3.5000e+00,
                              "Zeta": [1.],
                              "Lambda": [-1., 1.],
                              "EtaA": [0.08367770, 0.16857440, 0.50000000],
                              "MuA": [0.]
                           }
      vocabulary = ["H", "C", "N", "O",
                     "F", "S", "Cl"]
