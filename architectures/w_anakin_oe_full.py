...
class WeightedRadialAEVComputer(tf.keras.layers.Layer):
   ...
   def __init__(self,
      ...
      self.Rcr = radial_AEV_params["Rcr"]
      self.EtaR = radial_AEV_params["EtaR"]
      self.ShfR = radial_AEV_params["ShfR"]
...
class WeightedAngularAEVComputer(tf.keras.layers.Layer):
   ...
   def __init__(self,
      ...
      self.Rca = angular_AEV_params["Rca"]
      self.Zeta = angular_AEV_params["Zeta"]
      self.ShfZ = angular_AEV_params["ShfZ"]
      self.EtaA = angular_AEV_params["EtaA"]
      self.ShfA = angular_AEV_params["ShfA"]
...
class WAnakinOEFull(tf.keras.Model):
   """A version of WAnakinOE with the radial and angular AEV-length equal to the Obiwan's one and with 1.7M neurons like Anakin."""

   def __init__(self, max_molecule_size):
      super(WAnakinOEFull, self).__init__(name="w_anakin_oe_full")

      # torchani-2x hyperparameters
      num_radial_shifts = 128
      radial_AEV_params = {
                           "Rcr": tf.constant(5.1),
                           "EtaR": tf.constant([19.7]),
                           "ShfR": tf.linspace(start=0.8, stop=5.1-0.26875, num=num_radial_shifts)
                        }
      num_angular_shifts = 8
      num_coradial_shifts = 32
      angular_AEV_params = {
                              "Rca": tf.constant(3.5),
                              "Zeta": tf.constant([14.1]),
                              "ShfZ": tf.linspace(0.39269, 2.74889, num_angular_shifts),
                              "EtaA": tf.constant([12.5]),
                              "ShfA": tf.linspace(0.8, 3.5-0.3375, num_coradial_shifts)
                           }
      vocabulary = ["H", "C", "N", "O",
                     "F", "S", "Cl"]
      ...
      # Energy MLPs
      radial_aev_lenght = num_radial_shifts
      angular_aev_lenght = num_angular_shifts * num_coradial_shifts
      self.aev_lenght = int(radial_aev_lenght + angular_aev_lenght)
      self.networks = [self.getMLP([456, 220, 160]),
                        self.getMLP([424, 220, 160]),
                        self.getMLP([392, 180, 128]),
                        self.getMLP([392, 180, 128]),
                        self.getMLP([360, 150, 96]),
                        self.getMLP([360, 150, 96]),
                        self.getMLP([360, 150, 96])]   # (matched with the vocabulary)
